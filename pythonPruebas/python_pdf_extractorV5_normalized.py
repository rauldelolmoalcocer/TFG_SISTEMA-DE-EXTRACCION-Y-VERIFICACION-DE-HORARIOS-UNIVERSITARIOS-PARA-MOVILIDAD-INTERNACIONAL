...
def process_pdf(pdf_path):
    ...
    # IMPORTANTE:
    # Devolvemos tanto la información del PDF como sus registros
    # para poder construir después el JSON global.
    return {
        "file": {
            "path": pdf_path,
            "name": os.path.basename(pdf_path),
...
def main():
    ...
    global_result = {
        ...
        "summary": {
            "total_pdfs": len(processed_files),
            "total_records": len(global_records)
        },

        "input_files": processed_files,

        "standard_schedule": global_records
    }
    ...
    print("==========================================")
    print(f"PDFs procesados: {len(processed_files)}")
    print(f"Registros totales: {len(global_records)}")
    print(f"JSON global: {global_output_path}")
